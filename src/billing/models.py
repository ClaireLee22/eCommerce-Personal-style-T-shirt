...
import stripe
stripe.api_key = "sk_test_p5AKkH2t5JrbVIRhOOioFgRj00ouj9NZb5"
...
class BillingProfileManager(models.Manager):
    def new_or_get(self, request):
        ...
        if user.is_authenticated:
            # if user.email:
            "logged in user checkout; remeber payment stuff"
...
class CardManager(models.Manager):
    def all(self, *arg, **kwargs): # ModelClass.objects.all() --> ModelClass.objects.filter(active=True)
...
class Card(models.Model):
    ...
    stripe_id       = models.CharField(max_length=120) # required
    brand           = models.CharField(max_length=120, blank=True, null=True) # not required, can find on stripe
...
def new_card_post_receiver(sender, instance, created, *args, **kwargs):
    if instance.default: # if create, default=True
        billing_profile = instance.billing_profile
        qs = Card.objects.filter(billing_profile=billing_profile).exclude(pk=instance.pk)
        qs.update(default=False) # make current instance not as default
