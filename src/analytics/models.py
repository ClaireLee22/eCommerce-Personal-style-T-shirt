...
class ObjectViewed(models.Model):
    ...
    object_id       = models.PositiveIntegerField() # User id, producr id, Order id
...
def object_view_receiver(sender, instance, request, *arg, **kwargs):
...
object_viewed_signal.connect(object_view_receiver) # send sender along with actual signal
...
# def post_save_session_receiver(sender, instance, created, *arg, **kwargs):
#     if created:
...
def user_logged_in_receiver(sender, instance, request, *arg, **kwargs):
    print(instance)
    user = instance
    ip_address = get_client_ip(request)
